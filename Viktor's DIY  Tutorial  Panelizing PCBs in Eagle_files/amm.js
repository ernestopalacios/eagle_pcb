...
window.CHITIKA_ADS = window.CHITIKA_ADS ? window.CHITIKA_ADS : (function() {
    "use strict";
    var adhesion_container  = undefined;
    var adhesion_unit       = undefined;
    var already_bridged     = false;
    var debug_pixels        = (Math.random() < 0.1 ? true : false);
    var hover_container     = undefined;
    var jq                  = undefined;
    ...
    function drop_it_like_its_hot(response) {
        ...
        var url = 'http://' + CHITIKA.host + '/minimall' + unit.ad_url_params;
    ...
    var hover_config        = {
        ...
        'boxShadow'         : 'none',
        'cid'               : undefined,
        'close_handler'     : hover_hide,
        'close_height'      : '14px',
        'close_image'       : 'url(http://images.chitika.net/buttons/close_round_white_on_red.png)',
        'close_right'       : '4px',
        'close_top'         : '5px',
        'close_width'       : '16px',
        'height'            : 250,
    ...
    function adhesion() {
        ...
        if (!mobile_type || adhesion_unit) { return; }
        adhesion_container = top.document.getElementById('chitika-container-adhesion');
        if (adhesion_container) { return; }

        send_event('adhesion_activate', undefined, undefined);

        if (!window_data.top_accessible) { return; }
        if (top.document.cookie.indexOf("ch_adhesion_first_load") != -1) { return; }

        var config = CHITIKA_ADS.adhesion_config[mobile_type];
        if (!config) { return; }

        adhesion_container = create_container('chitika-container-adhesion', config, top.document);
        top.document.body.appendChild(adhesion_container);
        var frame = create_iframe('chitikaAdhesionAd', config.width, config.height, top.document);
        adhesion_container.appendChild(frame);
        adhesion_unit = {
            'cid'               : config.cid,
            'container'         : adhesion_container,
            'disable_vcpm'      : true,
            'fluidH'            : true,
            'frames'            : [ frame ],
        ...
        frame.contentWindow.render_ad = adhesion_callback;

        adhesion_container.style.bottom = '0px';
        adhesion_container.style.left = '0px';
        adhesion_container.style.height = config.height+'px';
    ...
    function adhesion_callback(response) {
        ...
        CHITIKA_ADS.adhesion_config[mobile_type].update_interval = setInterval(adhesion_update, 36);

        var metadata = {
            'xargs'         : adhesion_unit.navajo
        };
        send_event('adhesion_render', adhesion_unit.impId, metadata);
    }

    function adhesion_hide() {
        adhesion_container.style.visibility = "hidden";
        top.document.cookie = "ch_adhesion_first_load=1; expires=0; path=/";
        clearInterval(CHITIKA_ADS.adhesion_config[mobile_type()].update_interval);
        var metadata = {
            'xargs'         : adhesion_unit.navajo
        };
        send_event('adhesion_close', adhesion_unit.impId, metadata);
    }

    function adhesion_update() {
        var portrait = window.innerHeight > window.innerWidth;
        var ad_zoom = window.innerHeight/screen.height;
        var mobile_type = CHITIKA_ADS.mobile_type();
        var unit = adhesion_unit;
        // If the orientation or zoom have changed, resize the ad
        if (portrait !== CHITIKA_ADS.adhesion_config[mobile_type].portrait ||
            ad_zoom !== CHITIKA_ADS.adhesion_config[mobile_type].ad_zoom ||
            mobile_type === 1) {
        ...
        if (ad_zoom < CHITIKA_ADS.adhesion_config[mobile_type].min_zoom) {
            unit.container.style.display = 'none';
            CHITIKA_ADS.adhesion_config[mobile_type].zoom_hide = 1;
        } else if(CHITIKA_ADS.adhesion_config[mobile_type].zoom_hide == 1 && ad_zoom >= CHITIKA_ADS.adhesion_config[mobile_type].min_zoom) {
            unit.container.style.display = 'block';
            CHITIKA_ADS.adhesion_config[mobile_type].zoom_hide = 0;
        }
        CHITIKA_ADS.adhesion_config[mobile_type].ad_zoom = ad_zoom;
        CHITIKA_ADS.adhesion_config[mobile_type].portrait = portrait;
    }
    ...
    function hover() {
        var config = CHITIKA_ADS.hover_config;
        if (!window_data.top_accessible) { return; }
        if (top.document.cookie.indexOf("ch_hover_first_load") != -1) { return; }
        hover_container = top.document.getElementById('chitika-container-hover');
        if (hover_container) { return; }

        hover_container = create_container('chitika-container-hover', config, top.document);
        top.document.body.appendChild(hover_container);
        var frame = create_iframe('chitikaHoverAd', config.width, config.height, top.document);
        hover_container.appendChild(frame);
        var unit = {
            'cid'               : config.cid,
            'container'         : hover_container,
            'disable_vcpm'      : true,
            'frames'            : [ frame ],
            'height'            : config.height,
            'product'           : 'hover',
        ...
        write_empty_document(frame.contentWindow.document);
        frame.contentWindow.render_ad = hover_callback;
        hover_container.style.bottom = config.offsetBottom + 'px',
        hover_container.style.right = -config.width + 'px',
        CHITIKA.units.push(unit);
    }

    function hover_callback(response) {
        if (response === undefined ||
            (!response.output &&
             !response.alturl)) {
            hover_hide();
        ...
        frame.contentWindow.hover_config = CHITIKA_ADS.hover_config;
        render_ad_basic(response);
        render_ad_inject_content(response);
        CHITIKA_ADS.animate(hover_container,
                            50,
                            {'r': -CHITIKA_ADS.hover_config.width},
                            {'r': CHITIKA_ADS.hover_config.offsetRight});
    }

    function hover_hide() {
        hover_container.style.visibility = "hidden";
        top.document.cookie = "ch_hover_first_load=1; expires=0; path=/";
    }
    ...
    function make_ad_server_call() {
        var base_url = 'http://' + CHITIKA.host + '/minimall';
        for (var unit_id = 0; unit_id < CHITIKA.units.length; unit_id++) {
            var unit            = CHITIKA.units[unit_id];
    ...
    function sharing_is_caring(frame, steal_jquery) {
        ...
        w.adhesion_config = adhesion_config;
        w.CHITIKA = CHITIKA;
    ...
    return {
        ...
        'adhesion_config'           : adhesion_config,
...
if (window.chitika_units !== undefined) {
    CHITIKA.units = CHITIKA.units.concat(window.chitika_units);
